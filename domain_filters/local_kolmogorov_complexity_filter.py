...
def normalised_compression_distance_2d(
...
def normalised_compression_distance(
    spacetime: list[list[int]], x: int, y: int, neighbourhood_radius: int
) -> float:
    return NCD(
        x="".join(
            map(
...
def local_ncd_2d(spacetime: ndarray, neighbourhood_radius: int = 1) -> ndarray:
