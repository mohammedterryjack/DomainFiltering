...
from numpy import ndarray, zeros_like, mean
...
def coordinates_lightcone(
    ...
    for row_index,t_ in enumerate(rows):
        ...
        for x_ in left_overflow + list(range(leftmost_index, rightmost_index)) + right_overflow:        
            yield (t_,x_)

def cell_values(spacetime:ndarray, coordinates:list[tuple[int,int]]) -> Generator[int,None,None]:
    ...
    for spacetime in spacetimes:
        ...
        for t in range(max_time):
            for x in range(max_space):
                past_lightcone_repr = ''.join(map(str,cell_values(
                    spacetime=spacetime, 
                    coordinates=coordinates_lightcone(
                        x=x,t=t,max_space=max_space,max_time=max_time,
                        lightcone_depth=-lightcone_depth, allow_partial_cones=True
                    )
                )))
                future_lightcone = tuple(cell_values(
                    spacetime=spacetime,
                    coordinates=coordinates_lightcone(
                        x=x,t=t,max_space=max_space,max_time=max_time,
                        lightcone_depth=lightcone_depth, allow_partial_cones=False
                    )
                ))
                if any(future_lightcone):
                    past_lightcone_to_future_lightcones[past_lightcone_repr].add(future_lightcone)
...
def causal_states(
    ...
    for past_lightcone_repr,candidate_future_lightcones in past_lightcones.items():
        for causal_state, future_lightcones in causal_state_to_future_lightcones.items():
            if distance(
                ...
                causal_state_to_past_lightcones[causal_state].append(past_lightcone_repr)
        ...
        if past_lightcone_repr not in past_lightcone_to_causal_state:
            ...
            causal_state_to_future_lightcones[causal_state] = candidate_future_lightcones
...
def statistical_complexity(
    ...
    past_lightcone_to_causal_state,causal_state_to_statistical_complexity = causal_states(
    ...
    for past_lightcone_repr, causal_state in past_lightcone_to_causal_state.items():
        past_lightcone_to_statistical_complexity[past_lightcone_repr] = causal_state_to_statistical_complexity[causal_state]
...
def local_statistical_complexity_filter(
    ...
    for t in range(max_time):
        for x in range(max_space):
            ...
            filtered_spacetime[t,x] = past_lightcone_to_statistical_complexity.get(past_lightcone_repr,float("-inf"))
...
from cv2 import cvtColor, THRESH_BINARY, THRESH_OTSU, threshold
...
_, filtered_spacetime_bw = threshold((filtered_spacetime*255).astype("uint8"), 128, 255, THRESH_BINARY | THRESH_OTSU)
